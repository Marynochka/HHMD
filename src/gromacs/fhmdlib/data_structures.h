...
/*
 * end hhmd_G additions
 */

typedef struct tssl_data
{
    int         cells[4];
    float       cells_mu[4];
    double      cells_c[4][3];
    char        padding[128 - 2*sizeof(int) - 4*sizeof(float) - 12*sizeof(double)];
} tssl_data;
...
// hhMD additions:
    double      ro_mixed;
//    double      ro_fh_prev, ro_fh_deriv;
//    dvec        u_fh_prev, uro_fh_deriv;
    double      natoms;
    
    double         natoms_star;
    double      vol_star;
    double      mass_star, hybr_mass_star,  hybr_mass_star_without_mu, hybr_mass_star_1, hybr_mass_star_2, hybr_mass_star_3, hybr_mass_star_4;
    double      tot_mu, mu2m2_sum, mu2m_sum ;
    dvec 		hybr_vel_star, hybr_vel_star_without_mu, hybr_momentum_star;
    double      avg_mass_fh_particle, avg_mass_fh_particle_prev;
// end hhMD additions

    double      delta_ro;                   /* delta of MD and FH densities */
    dvec        grad_ro;                    /* grad of density */

    double      Sc;                         /* S parameter in the FH cell centres */
...
typedef struct FHMD
{
    ...
    FH_arrays  *arr_next;            /* FH/MD arrays */
    FH_arrays  *arr_prev;            /* FH/MD arrays */
    FH_arrays  *arr_temp;            /* FH/MD arrays */
    ...
    double      tau_m;          /* tau for Berendsen thermostat MARYNA */
    double      fluct_m;        /* magnitude of fluctuations  MARYNA */
    ...
    int         write_debug_frames;
    int         nsteps_temper;

    // hhMD additions:
    real        q;
    int         nat;
    dvec        *v_md;     // not used anymore
	dvec        *x_hybr;
    double      *ro_hybr;  // not used anymore
    ...
    double      *m_cross_term;
    dvec        *p_cross_term;
    double      *m_cross_term_prev;
    dvec        *p_cross_term_prev;
	double      *summu2;
	double		avg_Natoms_percell;
//    int         cells[27];
	double      ro_hybr_avg[10];
	double      ro_hybr_sd[10];
    Tesselation **tssl;
    int         *cells_map;
    int         tssl_npoints;
    double      hybr_scale_lin, hybr_scale_vol;
    double      fh_scale_lin, fh_scale_vol;
    
/*    int         cells[8000][4];
    double      cells_mu[8000][4];
    double      cells_c[8000][4][3];
*/    
    tssl_data   *cells_data;

	rvec        coords[8000];
//    int         atoms_near[8000][5];
    // end hhMD additions
} FHMD;
