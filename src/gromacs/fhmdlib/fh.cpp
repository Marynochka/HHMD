...
//        P_INIT = fh->FH_dens*(fh->FH_dens*(fh->FH_dens*EOS_A + EOS_B) + EOS_C) + EOS_D;
//        SOUND  = sqrt(3.0*fh->FH_dens*fh->FH_dens*EOS_A + 2.0*fh->FH_dens*EOS_B + EOS_C);
	P_INIT = EOS_A + EOS_B*exp(fh->FH_dens/EOS_C);
        SOUND  = sqrt(EOS_B*exp(fh->FH_dens/EOS_C)/EOS_C);
        break;
    case eos_spce:
        MU     = FHMD_EOS_SPCE_MU;
...
//    fh->std_rho = sqrt(fh->FH_temp*fh->FH_dens*FHMD_kB/(fh->box_volume*fh->q/(double)(fh->Ntot_md)))/SOUND;
//    fh->std_u   = sqrt(fh->FH_temp*FHMD_kB/(fh->FH_dens*fh->box_volume*fh->q/(double)(fh->Ntot_md)));

    fh->std_rho = sqrt(fh->FH_temp*fh->FH_dens*FHMD_kB/(fh->box_volume*fh->q/(double)(fh->nat)))/SOUND;
    fh->std_u   = sqrt(fh->FH_temp*FHMD_kB/(fh->FH_dens*fh->box_volume*fh->q/(double)(fh->nat)));

    if(MASTER(cr))
    {
        printf(MAKE_GREEN "FHMD: Analytical STD of density: %g [amu/nm^3], STD of velocity: %g [nm/ps]\n", fh->std_rho, fh->std_u);
        printf(MAKE_GREEN "FHMD: Initial Pressure = %g [bar], Speed of Sound = %g [m/s]\n", P_INIT*16.6054, SOUND*1000.0);
        printf(RESET_COLOR "\n");
    ...
    case One_Way:
        for(int i = 0; i < fh->Ntot; i++)
        {
            ...
            fh->arr_prev[i].ro_fh   = fh->FH_dens;
            fh->arr_prev[i].ro_fh_n = fh->FH_dens;
            fh->arr_prev[i].p       = P_INIT;
            fh->arr_prev[i].pn      = P_INIT;

            fh->arr_next[i].ro_fh   = fh->FH_dens;
            fh->arr_next[i].ro_fh_n = fh->FH_dens;
            fh->arr_next[i].p       = P_INIT;
            fh->arr_next[i].pn      = P_INIT;

            fh->arr_temp[i].ro_fh   = fh->FH_dens;
            fh->arr_temp[i].ro_fh_n = fh->FH_dens;
            fh->arr_temp[i].p       = P_INIT;
            fh->arr_temp[i].pn      = P_INIT;
            ...
            for(int d = 0; d < DIM; d++)
            {
                ...
                fh->arr_prev[i].rof[d]     = fh->FH_dens;
                fh->arr_prev[i].rofn[d]    = fh->FH_dens;
                fh->arr_prev[i].pf[d]      = P_INIT;
                fh->arr_prev[i].pfn[d]     = P_INIT;

                fh->arr_next[i].rof[d]     = fh->FH_dens;
                fh->arr_next[i].rofn[d]    = fh->FH_dens;
                fh->arr_next[i].pf[d]      = P_INIT;
                fh->arr_next[i].pfn[d]     = P_INIT;

                fh->arr_temp[i].rof[d]     = fh->FH_dens;
                fh->arr_temp[i].rofn[d]    = fh->FH_dens;
                fh->arr_temp[i].pf[d]      = P_INIT;
                fh->arr_temp[i].pfn[d]     = P_INIT;
            }
        }
        break;
...
//                        arr[C].p  = arr[C].ro_md_prime*(arr[C].ro_md_prime*(arr[C].ro_md_prime*EOS_A + EOS_B) + EOS_C) + EOS_D;
                        arr[C].p  = EOS_A+EOS_B*exp(arr[C].ro_md_prime/EOS_C);
                        arr[C].pn = arr[C].p;
                        break;
                    case eos_spce:
...
void FH_predictor(FHMD *fh)
{
    ...
    ivec   ind;
    double DT = fh->dt_FH;//*fh->hhmd_G_coeff;
...
//                    a[C].pn = a[C].ro_fh_n*(a[C].ro_fh_n*(a[C].ro_fh_n*EOS_A + EOS_B) + EOS_C) + EOS_D;
                    a[C].pn = EOS_A+EOS_B*exp(a[C].ro_fh_n/EOS_C);
                    break;
                case eos_spce:
...
//                    a[C].p = a[C].ro_fh*(a[C].ro_fh*(a[C].ro_fh*EOS_A + EOS_B) + EOS_C) + EOS_D;
                    a[C].p = EOS_A+EOS_B*exp(a[C].ro_fh/EOS_C);
                    break;
                case eos_spce:
...
//                    arr[C].pn = arr[C].ro_fh_n*(arr[C].ro_fh_n*(arr[C].ro_fh_n*EOS_A + EOS_B) + EOS_C) + EOS_D;
                    arr[C].pn = EOS_A+EOS_B*exp(arr[C].ro_fh_n/EOS_C);
                    break;
                case eos_spce:
...
//                    arr[C].p = arr[C].ro_fh*(arr[C].ro_fh*(arr[C].ro_fh*EOS_A + EOS_B) + EOS_C) + EOS_D;
                    arr[C].p = EOS_A+EOS_B*exp(arr[C].ro_fh/EOS_C);
                    break;
                case eos_spce:
...
void FH_char(FHMD *fh)
{
    ...
    const double DT  = fh->dt_FH;//*fh->hhmd_G_coeff;
...
//                        arr[C].pfn[d] = arr[C].rofn[d]*(arr[C].rofn[d]*(arr[C].rofn[d]*EOS_A + EOS_B) + EOS_C) + EOS_D;
                        arr[C].pfn[d] = EOS_A+EOS_B*exp(arr[C].rofn[d]/EOS_C);
                        break;
                    case eos_spce:
...
// {
// /*    for(int i = 0; i < fh->Ntot; i++) {
// 		fh->arr[i].ro_fh_prev = fh->arr[i].ro_fh;
   
// 	    for(int d = 0; d < DIM; d++)
// 			fh->arr[i].u_fh_prev[d] = fh->arr[i].u_fh[d];
// 	}
// */
// //    if (STEP>0)
// 	    for(int i = 0; i < fh->Ntot; i++)
// 	    	fh->arr_prev[i] = fh->arr[i];
...
// 	    for(int i = 0; i < fh->Ntot; i++) {
//     		fh->arr_next[i] = fh->arr[i];
// 	    	fh->arr[i] = fh->arr_temp[i];
// 		}   
//     }
// 	else {
	
// 	    for(int i = 0; i < fh->Ntot; i++) {
// 	    	fh->arr[i] = fh->arr_next[i];
// 	    	fh->arr_next[i] = fh->arr_temp[i];
// 	    }
// 	}
...
if (STEP == 0){
    ...
    if (STEP>0){
        ...
        // fprintf(mass_out, "step > 0 %d ", STEP);
    ...
    if (STEP!=0) {
	    ...
	    for(int i = 0; i < fh->Ntot; i++) {
            ...
            fh->arr[i].avg_mass_fh_particle= fh->arr_prev[i].avg_mass_fh_particle;
            fh->arr[i].ro_md= fh->arr_prev[i].ro_md;
            fh->arr[i].uro_md[0]= fh->arr_prev[i].uro_md[0];
            fh->arr[i].uro_md[1]= fh->arr_prev[i].uro_md[1];
            fh->arr[i].uro_md[2]= fh->arr_prev[i].uro_md[2];
...
//fprintf(stdout, "step %d - AAA %.5f %.5f %.5f \n", STEP, fh->arr_prev[0].ro_fh, fh->arr[0].ro_fh, fh->arr_next[0].ro_fh);
...
//fprintf(stdout, "step %d - BBB %.5f %.5f %.5f \n", STEP, fh->arr_prev[0].ro_fh, fh->arr[0].ro_fh, fh->arr_next[0].ro_fh);
...
//fprintf(stdout, "step %d - CCC %.5f %.5f %.5f \n", STEP, fh->arr_prev[0].ro_fh, fh->arr[0].ro_fh, fh->arr_next[0].ro_fh);
...
fprintf(stdout, "step %d - DDD %.5f %.5f %.5f \n", STEP-1, fh->arr_prev[0].ro_fh, fh->arr[0].ro_fh, fh->arr_next[0].ro_fh);
*/
}
