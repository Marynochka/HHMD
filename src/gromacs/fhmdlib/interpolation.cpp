...
void trilinear_interpolation_scalar(double &result, dvec xi, double f0, double f1, double f2, double f3, double f4, double f5, double f6, double f7)
{
    const double x = xi[0];
    const double y = xi[1];
    const double z = xi[2];

    result = (1-z)*((1-y)*((1-x)*f0 + x*f1) + y*((1-x)*f2 + x*f3)) +
                    (z)*((1-y)*((1-x)*f4 + x*f5) + y*((1-x)*f6 + x*f7));
}
