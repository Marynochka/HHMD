...
void FH_S_weighted(FHMD *fh)
{
    for(int i = 0; i < fh->Ntot; i++)
    {
        if(fh->S_function != constant_S)
            fh->arr[i].Sc = 1 - fh->arr[i].ro_md_s/fh->arr[i].ro_md;
        else
            fh->arr[i].Sc = fh->S;
    }
    ...
    for(int k = 0; k < NZ; k++)
    {
        for(int j = 0; j < NY; j++)
        {
            for(int i = 0; i < NX; i++)
            {
                ...
                for(int d = 0; d < DIM; d++)
                    fh->arr[L].Sf[d] = (fh->arr[CL].Sc + fh->arr[C].Sc)*0.5;
            }
        }
    }
}
        ...
        for(int j = fh->N_shift[1]; j < (fh->N_md[1] + fh->N_shift[1]); j++)
        {
            for(int i = fh->N_shift[0]; i < (fh->N_md[0] + fh->N_shift[0]); i++)
            {
                ...
                if(fh->S_function == moving_sphere)
                    fh->arr[C].Sc = fhmd_Sxyz_d(fh->grid.c[C], fh->protein_com, fh);         // MD/FH sphere follows protein
                else if(fh->S_function == fixed_sphere)
                    fh->arr[C].Sc = fhmd_Sxyz_d(fh->grid.c[C], fh->box05, fh);               // Fixed MD/FH sphere
                else
                    fh->arr[C].Sc = fh->S;                                                   // Constant S
...
void FH_S(FHMD *fh)
{
    ...
    {
        if(fh->S_function == moving_sphere)
            fh->arr[i].Sc = fhmd_Sxyz_d(fh->grid.c[i], fh->protein_com, fh);         // MD/FH sphere follows protein
        else if(fh->S_function == fixed_sphere)
            fh->arr[i].Sc = fhmd_Sxyz_d(fh->grid.c[i], fh->box05, fh);               // Fixed MD/FH sphere
        else
            fh->arr[i].Sc = fh->S;                                                   // Constant S

        if(fh->grid.md[i] == FH_zone)
            fh->arr[i].Sc = 1;                                                       // Pure FH zone
    }
