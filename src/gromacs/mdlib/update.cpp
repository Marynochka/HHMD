...
#include "gromacs/fhmdlib/macro.h"
...
#pragma omp parallel for num_threads(nth) schedule(static)
        for (th = 0; th < nth; th++)
            fhmd_calculate_MDFH_terms(x, v, mass, f, N_atoms, fh, th);
...
//		fh->arr[k].natoms_star += fh->cells_mu[n][q];
            }
	}


	    for (int n = 0; n < fh->Ntot; n++) {
			if (fh->arr[n].natoms_star==0)
			    continue;

	        fh->arr[n].avg_mass_fh_particle_prev = fh->arr[n].avg_mass_fh_particle;
            
            // fprintf(mass_out, "%f ", arr[n].avg_mass_fh_particle);
	        fh->arr[n].avg_mass_fh_particle = fh->arr[n].mass_star/fh->arr[n].natoms_star;
            // fprintf(mass_out, "%f ", arr[n].avg_mass_fh_particle);
            // fprintf(mass_out, "%f ", arr[n].mass_star);
            // fprintf(mass_out, "%d ", int(arr[n].natoms_star));
...
void fhmd_calculate_MDFH_terms_openmp(rvec x[], rvec v[], float mass[], rvec f[], int N_atoms, FHMD *fh)
{
    ...
    FH_arrays *arr = fh->arr;
	    ...
	    for (int n = 0; n < fh->Ntot; n++)
        {
	        ...
	        arr[n].mass_star = /* 0.25* */ ro_interp(fh,n) /*arr[n].ro_fh*/ *arr[n].vol_star;
	        arr[n].hybr_mass_star = 0;
	        arr[n].hybr_vel_star[0] = 0;
	        arr[n].hybr_vel_star[1] = 0;
	        arr[n].hybr_vel_star[2] = 0;

            arr[n].hybr_momentum_star[0] = 0;
	        arr[n].hybr_momentum_star[1] = 0;
	        arr[n].hybr_momentum_star[2] = 0;

            arr[n].hybr_mass_star_1 = 0;
            arr[n].hybr_mass_star_2 = 0;
            arr[n].hybr_mass_star_3 = 0;
            arr[n].hybr_mass_star_4 = 0;
            //MAryna
            // fprintf(stdout, "RHO in mass_star: K:%d, ro_interp: %.5f\n", n, ro_interp(fh, n));
            // fprintf(stdout, "mass of mass_star: K: %d, mass_star: %.5f\n", n, arr[n].mass_star);
    ...
    for (int q = 0; q < fh->tssl_npoints; ++q)
    {
      ...
      arr[k].mu2m2_sum = 0.0;
      arr[k].hybr_mass_star = 0.0;
      arr[k].hybr_mass_star_without_mu = 0.0;
      arr[k].tot_mu = 0.0;
      arr[k].hybr_momentum_star[0] = 0.0;
      arr[k].hybr_momentum_star[1] = 0.0;
      arr[k].hybr_momentum_star[2] = 0.0;
      arr[k].mu2m_sum = 0.0;
    }
...
#pragma omp parallel for num_threads(nth) schedule(static)
        for (th = 0; th < nth; th++)
        {
            int start_th = (N_atoms* th   )/nth;
            int end_th   = (N_atoms*(th+1))/nth;
            ...
            for (int n = start_th; n < end_th; n++)
            {
				...
				for (int q=0; q<fh->tssl_npoints; q++) {
				    ...
				    if ( fh->tssl[th]->in_this_cell(q,rsc) ) {
				    
						double mu_k = fh->tssl[th]->mu();
            					dvec c_k;
            					fh->tssl[th]->c(c_k);

					        if (nk < 4) {
/*							fh->cells   [n][nk]    = fh->cells_map[q];
							fh->cells_mu[n][nk]    = mu_k;
							fh->cells_c [n][nk][0] = c_k[0];
							fh->cells_c [n][nk][1] = c_k[1];
							fh->cells_c [n][nk][2] = c_k[2];
*/
							fh->cells_data [n].cells    [nk]    = fh->cells_map[q];
							fh->cells_data [n].cells_mu [nk]    = mu_k;
							fh->cells_data [n].cells_c  [nk][0] = c_k[0];
							fh->cells_data [n].cells_c  [nk][1] = c_k[1];
							fh->cells_data [n].cells_c  [nk][2] = c_k[2];
						}
						nk++;
				    }
				}
        ...
        {
//	  for (int i=0; i<nk; i++)
//		arr[fh->cells_data[n].cells[i]].natoms_star--;
...
/*	for (int n = N_atoms-20; n < N_atoms; n++)
	    for (int q = 0; q < 4; q++) {
		printf ("at%d,q%d,mu%.4f ",n,q,fh->cells_data[n].cells_mu[q]);
            }
	printf ("\n\n");
*/
