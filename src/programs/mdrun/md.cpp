                  ...
                  gmx_walltime_accounting_t walltime_accounting)
{
     ...
     * FHMD Initialization
     */
    int is_fhmd = fhmd_init(state->box, mdatoms->homenr, mdatoms->massT, state->x, state->v, ir->delta_t, top_global, cr, &fhmd);
    // maryna
    // FILE *mass_out;
    // mass_out = fopen("mass.dat", "a");
    // fprintf(mass_out, "\n ");
    // fprintf(mass_out, "%f ", fhmd.arr[0].avg_mass_fh_particle);
    // fclose(mass_out);
...
//				hhmd: calculate hybrid terms instead
                fhmd_calculate_MDFH_terms_openmp(state->x, state->v, mdatoms->massT, f, mdatoms->homenr, &fhmd);
...
/*
				//  hhmd_G: second time for ro_mixed
                fhmd_update_MD_in_FH2(state->x, state->v, mdatoms->massT, f, mdatoms->homenr, &fhmd);
                if(PAR(cr))
                {
                    fhmd_sum_arrays2(cr, &fhmd);                         // for selected arrays
                    gmx_bcast(sizeof(FH_arrays)*fhmd.Ntot, fhmd.arr, cr);   // actually we need this for the pure FH arrays only
                }
                    */ 
                //Maryna ?
